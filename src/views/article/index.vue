<template>
  <div>
    ...
    <el-card class="box-card">
      ...
      <!-- 数据分页
       page-size 配置煤业大小，默认是10
       total用来配置总记录数
       分页组件会根据每页大小和总记录数进行分页
       :disabled=true/false设置加载过程中禁止再次点击跳转
      -->
      <el-pagination
        background
        :current-page="page"
...
export default {
  ...
  data () {
    return {
      ...
      range_date: '' // 时间范围绑定值，这个字段的意义是为了绑定data组件触发 change事件
    }
  },
  created () {
    this.loadArticles()
  ...
  methods: {
    ...
    handleDateChange (value) {
      this.filterParams.begin_pubdata = value[0]
      this.filterParams.end_pubdata = value[1]
    },
    handleFilter () {
      // 默认每次查询新项目数据从第一页开始
      this.page = 1
      ...
      try {
        ...
        const data = await this.$http({
          ...
          params: {
            ...
            ...filterData // 将filterData 混入到当前对象中
          }
        })
